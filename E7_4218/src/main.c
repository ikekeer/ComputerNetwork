...
/*socks协议的版本，4或5*/
enum socks {
	RESERVED = 0x00,  //保留字段，恒为0
	VERSION4 = 0x04,  //sock4只支持TCP
	VERSION5 = 0x05   //sock5同时支持TCP和UDP
};

/*客户端请求连接时的验证方式*/
enum socks_auth_methods {
	NOAUTH = 0x00,   //不需要验证
	USERPASS = 0x02,  //用户名密码验证
	NOMETHOD = 0xff   //无支持的认证方法，无法连接
};

/*服务器端响应账号密码认证结果*/
enum socks_auth_userpass {
	AUTH_OK = 0x00,  //认证成功
	AUTH_VERSION = 0x01,  //认证子版本
	AUTH_FAIL = 0xff  //认证失败
};

/*客户端请求的类型*/
enum socks_command {
	CONNECT = 0x01  //连接上游服务器
};
...
int app_connect(int type, void *buf, unsigned short int portnum)
{
	...
	memset(address, 0, ARRAY_SIZE(address));  //address清零

	if (type == IP) {  //IP地址，绑定IP和端口
		char *ip = (char *)buf;
...
/*socks初始化，如果socks协议是版本4或5则继续，否则线程结束*/
...
int main(int argc, char *argv[])
{
	...
	signal(SIGPIPE, SIG_IGN);  //客户端关闭程序，程序结束
